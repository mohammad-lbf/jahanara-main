...
import data from '@/DB/Hamyar DB/sevom/darsnameh';
...
const Page = ({ currentFileData, data }) => {
  ...
  const pageKeywords = `درسنامه آموزشی, درسنامه پایه سوم, دبستان شهید جهان آرا, ${currentFileData.name}`;
  const canonicalUrl = `https://jahanaraschool.ir/hamyar/sevom/darsnameh/${currentFileData.id}`;
      ...
      <div style={{minHeight:"100vh"}} className='page-padding-tops'>
        <div style={{paddingTop:"20px" , paddingBottom:"20px"}} className='mt-5 mt-lg-2 container bg-white rounded shadow-sm border'>
          <div className='d-flex'>
            ...
            <i className="bi bi-chevron-left mx-2"></i>
            <Link href={"/hamyar/sevom"} style={{fontFamily:"KalamehWeb-Bold"}}>
              سوم
            </Link>
            <i className="bi bi-chevron-left mx-2"></i>
            <Link href={"/hamyar/sevom/darsnameh"} style={{fontFamily:"KalamehWeb-Bold"}}>
              درسنامه آموزشی
            </Link>
          </div>
        </div>
