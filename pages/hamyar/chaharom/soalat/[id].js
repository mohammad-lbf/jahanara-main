...
import data from '@/DB/Hamyar DB/chaharom/soalat';
...
const Page = ({ currentFileData, data }) => {
  ...
  const pageKeywords = `نمونه سوالات, سوالات پایه چهارم, دبستان شهید جهان آرا, ${currentFileData.name}`;
  const canonicalUrl = `https://jahanaraschool.ir/hamyar/chaharom/soalat/${currentFileData.id}`;
      ...
      <div style={{minHeight:"100vh"}} className='page-padding-tops'>
        <div style={{paddingTop:"20px" , paddingBottom:"20px"}} className='mt-5 mt-lg-2 container bg-white rounded shadow-sm border'>
          <div className='d-flex'>
            ...
            <i className="bi bi-chevron-left mx-2"></i>
            <Link href={"/hamyar/chaharom/soalat"} style={{fontFamily:"KalamehWeb-Bold"}}>
              نمونه سوالات
            </Link>
          </div>
        </div>
