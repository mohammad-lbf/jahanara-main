...
import data from '@/DB/Hamyar DB/sheshom/mohtava';
...
const Page = ({ currentFileData, data }) => {
  ...
  const pageKeywords = `محتوای آموزشی, محتوای آموزشی پایه ششم, دبستان شهید جهان آرا, ${currentFileData.name}`;
  const canonicalUrl = `https://jahanaraschool.ir/hamyar/sheshom/mohtava/${currentFileData.id}`;
      ...
      <Head>
        ...
        <div style={{minHeight:"100vh"}} className='page-padding-tops'>
            <div style={{paddingTop:"20px" , paddingBottom:"20px"}} className='mt-5 mt-lg-2 container bg-white rounded shadow-sm border'>
                ...
                <i class="bi bi-chevron-left mx-2"></i>
                <Link href={"/hamyar/sheshom/mohtava"} style={{fontFamily:"KalamehWeb-Bold"}}>
                    محتوای آموزشی
                </Link>
                </div>
            </div>
